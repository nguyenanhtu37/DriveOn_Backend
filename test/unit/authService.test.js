...
describe("authService.login()", () => {
  ...
  it("should throw error when email format is invalid", async () => {
    await expect(
      authService.login("invalidemail", "password123")
  ...
  it("should throw error when password length is less than 6 characters", async () => {
    await expect(
      authService.login("test@example.com", "12345")
    ).rejects.toThrow("Mật khẩu phải từ 6-50 ký tự");
  });
  ...
  it("should return token when login is successful", async () => {
    ...
    const result = await authService.login(
      "tunade170244@fpt.edu.vn",
      "tunade170244@fpt.edu.vn"
    );
    expect(result).toEqual({ token: "mocked_jwt_token" }); // Kiểm tra kết quả trả về
    expect(User.findOne).toHaveBeenCalledWith({
      email: "tunade170244@fpt.edu.vn",
    }); // Kiểm tra findOne có gọi đúng không
    expect(bcrypt.compare).toHaveBeenCalledWith(
      "tunade170244@fpt.edu.vn",
      mockUser.password
    ); // Kiểm tra so sánh mật khẩu có đúng không
