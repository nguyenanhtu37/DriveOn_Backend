...
const authMiddleware = async (req, res, next) => {
  try {
    ...
    if (!decoded.roles || !Array.isArray(decoded.roles)) {
      return res
        .status(403)
        .json({ message: "Access denied. Invalid roles structure." });
    }

    // Lọc bỏ các giá trị null và undefined trong mảng roles
    const validRoles = decoded.roles.filter(
      (role) => role !== null && role !== undefined
    );
    if (validRoles.length === 0) {
      return res
        .status(403)
        .json({ message: "Access denied. No valid roles found." });
    }
    ...
    if (user.status !== "active") {
      return res
        .status(403)
        .json({ message: "Access denied. User account is not active." });
    }
