...
export const updateProfileSchema = z.object({
  name: z
    .string({ required_error: "Tên phải là chuỗi ký tự" })
    .min(2, { message: "Tên phải có ít nhất 2 ký tự" })
    .optional(),
  
  phone: z
    .string()
    .regex(/^(84|0[3|5|7|8|9])[0-9]{8}$/, { message: "Số điện thoại không hợp lệ" })
    .optional(),
  
  bankAccount: z
    .string({ required_error: "Số tài khoản phải là chuỗi số" })
    .regex(/^\d+$/, { message: "Số tài khoản chỉ được chứa số" })
    .optional(),
  
  bankName: z
    .string({ required_error: "Tên ngân hàng phải là chuỗi ký tự" })
    .min(2, { message: "Tên ngân hàng không hợp lệ" })
    .optional(),
